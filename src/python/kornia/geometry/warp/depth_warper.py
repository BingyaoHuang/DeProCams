...
from kornia.geometry.camera import cam2pixel, pixel2cam, src2dst, cam2pixelNoTrans
...
class DepthWarper(nn.Module):
    ...
    def __init__(self,
        ...
        self.src_height: int = src_height
        self.dst_width: int = dst_width
        self.dst_height: int = dst_height
        self.mode: str = mode
        ...
        self._dst_trans_src: Union[None, torch.Tensor] = None  # -BH src -> dst extrinsics

        self.grid: torch.Tensor = self._create_meshgrid(src_height, src_width)
    ...
    def compute_projection_matrix(
        ...
        self._dst_trans_src: torch.Tensor = torch.matmul(self._pinhole_dst.extrinsics, pinhole_src.extrinsics.inverse())  # -BH

        # compute the projection matrix between the non reference cameras and
        # the reference.
        # dst_proj_src: torch.Tensor = torch.matmul(
        #     self._pinhole_dst.intrinsics, self._dst_trans_src)

        # update class members
        self._pinhole_src = pinhole_src
        self._dst_proj_src = torch.matmul(self._pinhole_dst.intrinsics, self._dst_trans_src)
        return self
    ...
    def compute_subpixel_step(self) -> torch.Tensor:
        ...
        """
        delta_d = 0.01
        xy_m1 = self._compute_projection(self.src_width / 2, self.src_height / 2,
                                         1.0 - delta_d)
        xy_p1 = self._compute_projection(self.src_width / 2, self.src_height / 2,
                                         1.0 + delta_d)
    ...
    def warp_grid(self, depth_src: torch.Tensor) -> torch.Tensor:
        ...
        cam_coords_src: torch.Tensor = pixel2cam(depth_src,
                                                 self._pinhole_src.intrinsics_inverse().to(device).to(dtype).expand((batch_size, 4, 4)),
                                                 # -BH add to device and expand
                                                 pixel_coords)  # BxHxWx3

        # reproject the camera coordinates to the pixel
        pixel_coords_src: torch.Tensor = cam2pixel(
            cam_coords_src, self._dst_proj_src.to(device).to(dtype).expand((batch_size, 4, 4)))  # -BH add to device and expand. (B*N)xHxWx2

        # normalize between -1 and 1 the coordinates
        pixel_coords_src_norm: torch.Tensor = normalize_pixel_coordinates(pixel_coords_src, self.dst_height, self.dst_width)
        return pixel_coords_src_norm

    # also return the 3d coords in dst view space
    def warp_grid2(self, depth_src: torch.Tensor) -> torch.Tensor:
        """Computes a grid for warping a given the depth from the reference
        pinhole camera.
        ...
        # this two-step method is slower than the method cam2pixel above, but we need cam_coords_dst for indirect and directly light estimation. -BH
        cam_coords_dst: torch.Tensor = src2dst(cam_coords_src,
                                               self._dst_trans_src.to(device).to(dtype).expand(
                                                   (batch_size, 4, 4)))  # -BH add to device and expand. (B*N)xHxWx2

        pixel_coords_src: torch.Tensor = cam2pixelNoTrans(cam_coords_dst,
                                                          self._pinhole_dst.intrinsics.to(device).to(dtype).expand(
                                                              (batch_size, 4, 4)))  # -BH add to device and expand. (B*N)xHxWx2

        # normalize between -1 and 1 the coordinates
        pixel_coords_src_norm: torch.Tensor = normalize_pixel_coordinates(pixel_coords_src, self.dst_height, self.dst_width)
        return pixel_coords_src_norm, cam_coords_dst
    ...
    def forward(  # type: ignore
        ...
        """
        return F.grid_sample(patch_dst, self.warp_grid(depth_src), mode=self.mode, padding_mode=self.padding_mode)

        # dst_flow_src, cam_coords_dst = self.warp_grid2(depth_src)
        # return F.grid_sample(patch_dst, dst_flow_src, mode=self.mode,
        #                      padding_mode=self.padding_mode), dst_flow_src, cam_coords_dst  # also return the flow and coord in dst view space. -BH

    # use a single warp grid to warp all images
    def forward_fast(  # type: ignore
            self,
            depth_src: torch.Tensor,
            patch_dst: torch.Tensor) -> torch.Tensor:
        """Warps a tensor from destination frame to reference given the depth
        ...
        # use single grid to warp all, faster
        # dst_flow_src, cam_coords_dst = self.warp_grid2(depth_src)
        # return F.grid_sample(patch_dst, dst_flow_src.expand(patch_dst.shape[0], *dst_flow_src.shape[1:4]), mode=self.mode,
        #                      padding_mode=self.padding_mode), dst_flow_src, cam_coords_dst  # also return the flow and coord in dst view space. -BH

        dst_flow_src = self.warp_grid(depth_src)
        return F.grid_sample(patch_dst, dst_flow_src.expand(patch_dst.shape[0], *dst_flow_src.shape[1:4]), mode=self.mode,
...
def depth_warp(pinhole_dst: PinholeCamera,
    ...
    warper = DepthWarper(pinhole_dst, src_height, src_width, dst_height, dst_width) # new DepthWarper input. -BH
    warper.compute_projection_matrix(pinhole_src)
