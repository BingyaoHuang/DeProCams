...
# projection only, no extrinsics transformation, use src2dst in this function. -BH
def cam2pixelNoTrans(
        cam_coords: torch.Tensor, # coordinates in the current camera's view space
        proj_mat: torch.Tensor, # intrinsic matrix of the current camera
        eps: Optional[float] = 1e-6) -> torch.Tensor:
    r"""Transform coordinates in the camera frame to the pixel frame.
    ...
    Args:
        cam_coords (torch.Tensor): pixel coordinates defined in the first
          camera coordinates system. Shape must be BxHxWx3.
        proj_mat (torch.Tensor): the projection matrix between the
          reference and the non reference camera frame. Shape must be Bx4x4.

    Returns:
        torch.Tensor: array of [-1, 1] coordinates of shape BxHxWx2.
    """
    if not len(cam_coords.shape) == 4 and cam_coords.shape[3] == 3:
        raise ValueError("Input cam_coords_src has to be in the shape of "
                         "BxHxWx3. Got {}".format(cam_coords.shape))
    if not len(proj_mat.shape) == 3 and proj_mat.shape[-2:] == (4, 4):
        raise ValueError("Input dst_proj_src has to be in the shape of "
                         "Bx4x4. Got {}".format(proj_mat.shape))
    b, h, w, _ = cam_coords.shape

    # apply projection matrix to points
    point_coords: torch.Tensor = transform_points(
        proj_mat[:, None], cam_coords)
    x_coord: torch.Tensor = point_coords[..., 0]
    y_coord: torch.Tensor = point_coords[..., 1]
    z_coord: torch.Tensor = point_coords[..., 2]
    ...
    # stack and return the coordinates, that's the actual flow
    pixel_coords: torch.Tensor = torch.stack([u_coord, v_coord], dim=-1)
    return pixel_coords  # (B*N)xHxWx2

# transform 3d points in src view space to dst view space using src and dst extrinsics (no projection). -BH
def src2dst(
        cam_coords_src: torch.Tensor,
    ...
    Args:
        cam_coords (torch.Tensor): pixel coordinates defined in the first
          camera coordinates system. Shape must be BxHxWx3.
        dst_trans_src (torch.Tensor): the projection matrix between the
          reference and the non reference camera frame. Shape must be Bx4x4.
    ...
    if not len(cam_coords_src.shape) == 4 and cam_coords_src.shape[3] == 3:
        raise ValueError("Input cam_coords_src has to be in the shape of "
                         "BxHxWx3. Got {}".format(cam_coords_src.shape))
    if not len(dst_trans_src.shape) == 3 and dst_trans_src.shape[-2:] == (4, 4):
        raise ValueError("Input dst_proj_src has to be in the shape of "
                         "Bx4x4. Got {}".format(dst_trans_src.shape))
    b, h, w, _ = cam_coords_src.shape

    # apply projection matrix to points
    point_coords: torch.Tensor = transform_points(
        dst_trans_src[:, None], cam_coords_src)
    x_coord: torch.Tensor = point_coords[..., 0]
    y_coord: torch.Tensor = point_coords[..., 1]
    z_coord: torch.Tensor = point_coords[..., 2]

    # stack and return the coordinates in dst cam view space
    cam_coords_dst: torch.Tensor = torch.stack([x_coord, y_coord, z_coord], dim=-1)
    return cam_coords_dst  # (B*N)xHxWx2
